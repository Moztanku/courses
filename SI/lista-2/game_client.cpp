...
int diagonalsBlocked(const int position[5][5], int player){
    int score = 0;
    for(int x = 0; x < 4; x++)
        if(position[x][x] == player){
            score += 1;
            break;
        }
    for(int x = 1; x < 5; x++)
        if(position[x-1][x] == player){
            score += 1;
            break;
        }
    for(int x = 0; x < 4; x++)
    ...
    for(int x = 0; x < 4; x++)
        if(position[4-x][x] == player){
            score += 1;
            break;
        }
    for(int x = 1; x < 5; x++)
        if(position[5-x][x] == player){
            score += 1;
            break;
        }
    for(int x = 0; x < 4; x++)
        if(position[3-x][x] == player){
            score += 1;
            break;
        }
    for(int x = 1; x < 5; x++)
...
int checkCorners(const int position[5][5], int player){
    int score = 0;
    if(position[2][2] == player)
        score += 2;
    if(position[0][0] == player)
        score += 1;
    if(position[0][4] == player)
        score += 1;
    if(position[4][0] == player)
        score += 1;
    if(position[4][4] == player)
        score += 1;
    return score / 2;
}

int checkThreats(const int position[5][5], int player){
    int score = 0;
    for(int i = 0; i < 48; i++){
        int count = 0;
        if(position[win[i][0][0]][win[i][0][1]] == player)
            count += 1;
        else if(position[win[i][0][0]][win[i][0][1]] == opponent)
            continue;
        if(position[win[i][1][0]][win[i][1][1]] == player)
            count += 1;
        else if(position[win[i][1][0]][win[i][1][1]] == opponent)
            continue;
        if(position[win[i][2][0]][win[i][2][1]] == player)
            count += 1;
        else if(position[win[i][2][0]][win[i][2][1]] == opponent)
            continue;
        if(position[win[i][3][0]][win[i][3][1]] == player)
            count += 1;
        else if(position[win[i][3][0]][win[i][3][1]] == opponent)
            continue;
...
int evaluate(const int position[5][5]){
    ...
    const int threatWeight = 1;
    happy += threatWeight*checkThreats(position, player);
    sad += threatWeight*checkThreats(position, opponent);

    const int blockWeight = 2;
    happy += blockWeight*(columnsBlocked(position, player) + 
             rowsBlocked(position, player) + 
             diagonalsBlocked(position, player));
    
    sad += blockWeight*(columnsBlocked(position, opponent) +
